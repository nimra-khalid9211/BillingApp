...
export const MyBills = ({from}) => {
  return (
    ...
    <div className="bg-[#377CF6] text-white p-3">{from === "BMB" ? "My Bills" : "Due Bills List"}</div>
...
export const SelectableBills =({type})=>{
    ...
    const totalAmount = selectedBills.reduce((total, bill) => {
        const amountString = bill.status === "unpaid" && new Date(bill.dueDate) < new Date()  ? bill.afterDueAmount : bill.amount;
        ...
        <tbody>
          ...
          {filteredBills.map((x, index)=>( 
             <tr key={index} className="bg-white border-dashed border-b border-[#eff0fb] text-center h-16 text-black font-medium">
                ...
                <td className={`px-6 py-2 ${dashedBorderRight}`}>
                    {new Date(x.dueDate) < new Date() && x.status === "unpaid" ? x.afterDueAmount : x.amount}
                </td>
                <td className={`px-6 py-2 ${dashedBorderRight}`}>
                <Switch defaultChecked={x.autoDebit} />
                </td>
              ...
              <Button className='px-1'>  <FiDownload size={20} color='#329DFF'/> </Button>  
                ...
                <td className={`px-6 py-2 ${dashedBorderRight}`}>
                  ...
                  <div className='flex flex-col justify-center items-center'>
                    ...
                    {new Date(x.dueDate) < new Date() ? <span className='text-[#F1416C] pt-1 text-xs'>Overdue</span>:""}
                </div>
                        :
                <div className='flex items-center justify-evenly px-6 py-2 '>
                
                    <div className='p-2 bg-[#DBF0FF] rounded-lg'><IoMdEye size={20} /></div>
                    
                    <div className='p-2 bg-[#DBF0FF] rounded-lg'><FaShareNodes size={20}/></div>
